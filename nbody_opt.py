...
def update_vs(v1, v2, dt, dx, dy, dz, m1, m2):
    mag = dt * ((dx * dx + dy * dy + dz * dz) ** (-1.5))
    v1[0] -= dx * m2 * mag #compute_b(m2, dt, dx, dy, dz)
    v1[1] -= dy * m2 * mag #compute_b(m2, dt, dx, dy, dz)
    v1[2] -= dz * m2 * mag #compute_b(m2, dt, dx, dy, dz)
    v2[0] += dx * m1 * mag #compute_b(m1, dt, dx, dy, dz)
    v2[1] += dy * m1 * mag #compute_b(m1, dt, dx, dy, dz)
    v2[2] += dz * m1 * mag #compute_b(m1, dt, dx, dy, dz)
